...
import com.typesafe.sbt.web.incremental.{OpFailure, OpInputHash, OpInputHasher, OpResult, OpSuccess}
...
object SbtJsTask extends AutoPlugin {
    ...
    val _jsOptions = Def.task(jsOptions.value)

    val jsTaskSpecificUnscopedConfigSettings = Seq(
        fileInputHasher := OpInputHasher[File](f => OpInputHash.hashString(f.getAbsolutePath + "|" + _jsOptions)),
        ...
        val results: (Set[File], Seq[Problem]) = incremental.syncIncremental((streams in config).value.cacheDirectory / "run", sources) {
            modifiedSources: Seq[File] =>
                ...
                if (modifiedSources.size > 0) {
                    ...
                    Def.task {
                        streams.value.log.info(s"${_taskMessage} on ${
                            modifiedSources.size
                        } source(s)")
                    }

                    val resultBatches: Seq[Future[(FileOpResultMappings, Seq[Problem])]] = {
                        val sourceBatches = (modifiedSources grouped Math.max(modifiedSources.size / _parallelism, 1)).toSeq
                        sourceBatches.map {
                            sourceBatch =>
                                withActorRefFactory(_state, this.getClass.getName) {
                                    arf =>
                                        val engine = arf.actorOf(engineProps)
                                        implicit val timeout = Timeout(_timeoutPerSource * modifiedSources.size)
                                        executeSourceFilesJs(
                                            engine,
                                            _shellSource,
                                            sourceBatch.pair(relativeTo(_sourceDirectories)),
                                            _resourceManaged,
                                            __jsOptions,
                                            m => logger.error(m),
                                            m => logger.info(m)
                    ...
                    val completedResults = Await.result(pendingResults, _timeoutPerSource * modifiedSources.size)
